...
There is **no public sign-up form anywhere in this app** — that's intentional, so you're the
only person who can ever have an account. You create your one admin login directly in
Supabase:

1. In the Supabase dashboard, go to **Authentication -> Users -> Add user -> Create new user**.
2. Enter the email and password you want to sign in with, and make sure **"Auto Confirm User"**
   is checked (so it doesn't wait on a confirmation email). Click **Create user**.
3. In **SQL Editor**, open `supabase/promote-to-admin.sql`, replace `you@example.com` with the
   email you just created, and run it. This is what actually grants admin access — creating the
   user alone is not enough, since new accounts default to role `user` at the database level.
4. Go to `http://localhost:3000/admin/login` and sign in with that email/password. You now have
   full admin access.

**For extra safety**, also turn off public sign-ups at the project level so nobody could ever
create an account even by calling the Supabase Auth API directly: in Supabase, go to
**Authentication -> Providers -> Email** and disable **"Allow new users to sign up."** Since you
create your admin user manually as described above, this doesn't stop you — it only blocks
everyone else.

If you ever need a second admin (e.g. a co-founder), repeat steps 1–3 with their email.
